...
def draw_eyes(image, lms, eyes, draw_eyeball=True, draw_iris=True, draw_rings=True, colour=[178, 255, 102],squint_factor=1.0,thickness_multiplier=1.0,color_mode="normal"):
    '''
    Input args:
        image: cv2 image
    ...
    draw_iris_solid = False
    if color_mode == "solid":
        draw_iris_solid = True
    elif color_mode == "inverted":
        colour_iris = colour_iris[::-1]
        colour_eyeball = colour[::-1]
        colour_rings = colour[::-1]
    ...
    if lms5.shape[0] == 68:
        lms5 = kpt5_from_68(lms5)
    diag = get_face_diag_from_lms5(lms5)
    base_thickness_iris = 2 if max(int(diag / 100), 1) < 3 else 3
    base_thickness_eyeball = 2 if max(int(diag / 100), 1) < 3 else 3
    base_thickness_rings = 1 if max(int(diag / 100), 1) < 3 else 2
    
    # Apply thickness multiplier and ensure minimum thickness of 1
    thickness_iris = max(1, int(base_thickness_iris * thickness_multiplier))
    thickness_eyeball = max(1, int(base_thickness_eyeball * thickness_multiplier))
    thickness_rings = max(1, int(base_thickness_rings * thickness_multiplier))
    ...
    if draw_eyeball:
        for side, eye in eyes.items():
            ...
            center = tuple(cnt[:2].astype(np.int32).tolist())
            
            if squint_factor >= 1.0:
                # Normal circle when squint_factor >= 1.0
                image = cv2.circle(
                    image, center, radius, colour_eyeball, thickness_eyeball)
            else:
                # Squinted ellipse when squint_factor < 1.0
                # Keep x-axis (width) the same, scale y-axis (height) by squint_factor
                axes = (radius, int(radius * squint_factor))  # (width, height)
                angle = 0  # No rotation
                start_angle = 0
                end_angle = 360
                
                image = cv2.ellipse(
                    image, center, axes, angle, start_angle, end_angle, 
                    colour_eyeball, thickness_eyeball)
    # draw rings
    if draw_rings:
        for side, eye in eyes.items():
            # eye = eye[:, [1, 0]]
